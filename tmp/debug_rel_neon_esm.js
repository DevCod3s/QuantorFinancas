...
async function debugData() {
  ...
  try {
    console.log("--- DIAGNÓSTICO DE DADOS NO NEON (ESM) ---");
    ...
    const relRes = await client.query('SELECT id, user_id, type, social_name, city, state, zip_code FROM relationships');
    console.log(`Total de relacionamentos cadastrados: ${relRes.rowCount}`);
    if (relRes.rowCount > 0) {
      console.log("Amostra de dados:");
      console.table(relRes.rows.slice(0, 50));
    }
